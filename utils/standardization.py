...
def get_von_mises_rms(mol, mol_rdkit, rotable_bonds, conf_id):
    ...
    #     reindex = mol.GetSubstructMatch(mol_rdkit)
    #     Chem.MolToMolFile(mol, './draw/mol.sdf')
    #     Chem.MolToMolFile(mol_rdkit, './draw/raw_mol_rdkit22.sdf')
    #     print(e)

def get_von_mises_rms_custom(mol, mol_rdkit, rotable_bonds, conf_id, template_conf_id):
    new_dihedrals = np.zeros(len(rotable_bonds))
    for idx, r in enumerate(rotable_bonds):
        new_dihedrals[idx] = get_dihedral_vonMises(mol_rdkit,
                                                   mol_rdkit.GetConformer(conf_id), r,
                                                   mol.GetConformer(template_conf_id).GetPositions())

    mol_rdkit = apply_changes(mol_rdkit, new_dihedrals, rotable_bonds, conf_id)
    return RMSD(mol_rdkit, mol,  prbCid=conf_id, refCid=template_conf_id)

# def get_von_mises_rms_custom(mol,template_rdkit, mol_rdkit, rotable_bonds, conf_id, reindex):
#
#     # 创建新的构象
#     conf_new = Chem.Conformer(mol.GetNumAtoms())
#     positions = mol.GetConformer().GetPositions()
#     for rdkit_ind, m_id in enumerate(reindex):
#         conf_new.SetAtomPosition(rdkit_ind, positions[m_id])
#
#     template_rdkit.RemoveAllConformers(conf_id)
#     template_rdkit.AddConformer(conf_new)
#
#
#     new_dihedrals = np.zeros(len(rotable_bonds))
#     for idx, r in enumerate(rotable_bonds):
