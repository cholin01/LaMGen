...
def write_sdf(mol, sdf_file, verbose=0):
    try:
        ...
        print(f"Error saving SDF file: {e}")
...
def prepare_ligand(work_dir, lig_sdf, verbose=1):
    ...
    command = f'''/home/gouqiaolin/miniconda3/envs/esm/bin/obabel {lig_sdf} -O {work_mol2}'''
...
def docking_with_sdf(work_dir, protein_pdbqt, lig_pdbqt, centroid, verbose=0, out_lig_sdf=None, save_pdbqt=False):
    ...
    command = '''qvina2 \
        ...
        /home/gouqiaolin/miniconda3/envs/esm/bin/obabel {out_lig_pdbqt} -O {out_lig_sdf} -h'''.format(
...
def docking_in_gpu(work_dir, protein_pdbqt, lig_pdbqt, centroid, verbose=0, out_lig_sdf=None, save_pdbqt=False):
    ...
    command2 = f'/home/gouqiaolin/miniconda3/envs/esm/bin/obabel {out_lig_pdbqt} -O {out_lig_sdf} -h'
...
def dual_docking_score(mol, protein_dir, rank=None, save_sdf=False, use_gpu=False) -> float:

    results = []
    for i in [0, 1]:
        protein_dir = Path(protein_dir) # Path with protein and ligands files
        protein_dir.mkdir(parents=True, exist_ok=True)
        protein_file = str(list(protein_dir.glob('*.pdb'))[i])
        print(protein_file)
        protein_filename = protein_file.split('/')[-1]
        protein_name = protein_filename.split('.')[0]
        ori_lig_file = f'{protein_name}-ligand.sdf' # We suggest naming the ori ligand like this to avoid mistakes
        ori_lig_file = os.path.join(protein_dir, ori_lig_file)

        # Prepare protein
        protein_pdbqt = prepare_target(protein_dir, protein_filename)
