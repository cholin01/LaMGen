...
def init_logging(rank):
    ...
    if rank == 0:
        file_handler = logging.FileHandler('./2tar_train.log')
...
def train(rank, args, world_size):
    ...
    model = LaMGen_dual(pretrain_path=args.model_path, config=Ada_config).to(torch.float32).to(device)
    model.to(rank)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if rank == 0 and epoch >= 1:
            torch.save(model.state_dict(), f"./checkpoint/dual/ckpt_{epoch}")
...
def evaluate(model, dataloader, args):
    ...
    with torch.no_grad():
        for mix_batch in dataloader:
            ...
            total_loss = loss_conf*0.2 + loss_smiles*0.8
            total_acc = acc_conf*0.2 + acc_smiles*0.8
